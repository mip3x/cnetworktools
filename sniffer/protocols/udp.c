...
void process_udp_header(struct state* state) {
    ...
    fprintf(state->log_file, "\nUDP Header\n");
    fprintf(state->log_file, "\t|-Source Port\t\t\t: %" PRIu16 " (0x%" PRIx16 ")\n", ntohs(udp->source), ntohs(udp->source));
    fprintf(state->log_file, "\t|-Destination Port\t\t: %" PRIu16 " (0x%" PRIx16 ")\n", ntohs(udp->dest), ntohs(udp->dest));
    fprintf(state->log_file, "\t|-UDP Length\t\t\t: %" PRIu16 " (0x%" PRIx16 ")\n", ntohs(udp->len), ntohs(udp->len));
